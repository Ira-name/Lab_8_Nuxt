<template>
  <div class="w-1/3 m-auto">
    ...
    <div class="p-5 border-2 rounded">
      <h1 class="text-center mb-2">Edit category {{category?.title}}</h1>
...
const getCategory = async (id: number) => {
  try {
    const response = await $fetch<Category>(`http://127.0.0.1:8000/api/categories/${id}`);
...
const getCategories = async () => {
  try {
    const response = await $fetch<Category[]>(`http://127.0.0.1:8000/api/categories`);
...
async function onSubmit (event: Event) {
  try {
    ...
    const response = await axios.put(`http://127.0.0.1:8000/api/categories/${categoryId.value}`, dataToSend);
